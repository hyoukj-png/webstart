...
| Playwright (검수용) | `npx playwright --version` | `npx playwright install chromium` |
...
# 3. Playwright 브라우저 설치 (검수 파이프라인용)
...
/webstart    # → 프로젝트 세팅 안내 출력
/audit       # → 분석 대상 URL 요청 출력
```
...
각 단계는 이전 단계 완료를 자동 확인합니다 (게이트 체크).

**3. 특정 단계만 실행**

```
/audit --step=ux https://example.com   # UX 분석만
/audit --step=tech https://example.com # 기술 분석만
```
...
Claude Code CLI 외에 claude.ai 웹에서도 사용하려면 Projects를 설정합니다.
